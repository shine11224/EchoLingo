...
- Local translation — on Windows x64, Settings provides a one-click installer for Tencent HY-MT1.5 Q4_K_M and llama.cpp b10068. It downloads pinned files from the official sources, verifies SHA-256, and then generates Chinese subtitles without `HY_TRANSLATE_API_KEY`. The model is governed by the [Tencent HY Community License](https://github.com/Tencent-Hunyuan/HY-MT/blob/main/License.txt) (not available in the EU/UK/South Korea); see `THIRD_PARTY_LICENSES.md`. macOS/Linux users can install `llama-server` from the official llama.cpp release and place the same GGUF in `models/`
